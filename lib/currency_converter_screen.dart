import 'package:flutter/material.dart';
import 'package:intl/intl.dart';


class CurrencyConverterScreen extends StatefulWidget {
  @override
  _CurrencyConverterScreenState createState() => _CurrencyConverterScreenState();
}

class _CurrencyConverterScreenState extends State<CurrencyConverterScreen> {
  double inputAmount = 0;
  double outputAmount = 0;
  String selectedCurrencyFrom = 'Rupiah';
  String selectedCurrencyTo = 'USD';
  String selectedTimeZone = 'WIB';
  DateTime selectedTime = DateTime.now();
  List<String> timeZones = [
    'WIB',
    'WIT',
    'WITA',
    'Singapore',
    'Sydney',
    'New York',
    'London',
    'Moscow',
    'Guangzhou',
    'Tokyo',
    'Seoul',
  ];

  Map<String, double> conversionRates = {
    'Rupiah': 10000,
    'USD': 0.67,
    'USD Singapore': 0.90,
    'USD Australia': 1.01,
    'Yen': 92.37,
    'Poundsterling': 0.54,
    'Euro': 0.62,
  };

  void convertCurrency() {
    double conversionRateFrom = conversionRates[selectedCurrencyFrom] ?? 1;
    double conversionRateTo = conversionRates[selectedCurrencyTo] ?? 1;

    setState(() {
      outputAmount = inputAmount * (conversionRateTo / conversionRateFrom);
    });
  }

  String convertTime(String timeZone) {
    DateTime convertedTime;

    switch (timeZone) {
      case 'WIT':
        convertedTime = selectedTime.add(Duration(hours: 2));
        break;
      case 'WITA':
        convertedTime = selectedTime.add(Duration(hours: 1));
        break;
      case 'Singapore':
        convertedTime = selectedTime.add(Duration(hours: 1));
        break;
      case 'Sydney':
        convertedTime = selectedTime.add(Duration(hours: 3));
        break;
      case 'New York':
        convertedTime = selectedTime.add(Duration(hours: -12));
        break;
      case 'London':
        convertedTime = selectedTime.add(Duration(hours: -6));
        break;
      case 'Moscow':
        convertedTime = selectedTime.add(Duration(hours: -4));
        break;
      case 'Guangzhou':
        convertedTime = selectedTime.add(Duration(hours: 1));
        break;
      case 'Tokyo':
        convertedTime = selectedTime.add(Duration(hours: 2));
        break;
      case 'Seoul':
        convertedTime = selectedTime.add(Duration(hours: 2));
        break;
      default:
        convertedTime = selectedTime;
    }
    // Gunakan DateFormat untuk memformat waktu yang ditampilkan
    DateFormat formatter = DateFormat.Hms(); // Format jam, menit, detik
    String formattedTime = formatter.format(convertedTime);

    return formattedTime.toString();
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Currency and Time Converter'),
      ),
      body: SingleChildScrollView(
        child: Padding(
          padding: const EdgeInsets.all(16.0),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.stretch,
            children: [
              Text(
                'Convert Currency',
                style: TextStyle(
                  fontSize: 18.0,
                  fontWeight: FontWeight.bold,
                ),
              ),
              SizedBox(height: 8.0),
              Row(
                children: [
                  Expanded(
                    child: LayoutBuilder(
                      builder: (BuildContext context, BoxConstraints constraints) {
                        return Container(
                          padding: EdgeInsets.symmetric(horizontal: 12.0),
                          decoration: BoxDecoration(
                            borderRadius: BorderRadius.circular(8.0),
                            border: Border.all(color: Colors.grey),
                          ),
                          child: DropdownButton<String>(
                            value: selectedCurrencyFrom,
                            onChanged: (String? newValue) {
                              setState(() {
                                selectedCurrencyFrom = newValue ?? '';
                              });
                            },
                            items: conversionRates.keys.map((String currency) {
                              return DropdownMenuItem<String>(
                                value: currency,
                                child: Text(currency),
                              );
                            }).toList(),
                            // Setelah DropdownButton dibangun, kita bisa mengatur lebarnya sesuai dengan constraints yang tersedia
                            // dengan menambahkan properti "isExpanded" menjadi true.
                            isExpanded: true,
                          ),
                        );
                      },
                    ),
                  ),
                  SizedBox(width: 16.0),
                  Icon(Icons.arrow_forward),
                  SizedBox(width: 16.0),
                  Expanded(
                    child: LayoutBuilder(
                      builder: (BuildContext context, BoxConstraints constraints) {
                        return Container(
                          padding: EdgeInsets.symmetric(horizontal: 12.0),
                          decoration: BoxDecoration(
                            borderRadius: BorderRadius.circular(8.0),
                            border: Border.all(color: Colors.grey),
                          ),
                          child: DropdownButton<String>(
                            value: selectedCurrencyTo,
                            onChanged: (String? newValue) {
                              setState(() {
                                selectedCurrencyTo = newValue ?? '';
                              });
                            },
                            items: conversionRates.keys.map((String currency) {
                              return DropdownMenuItem<String>(
                                value: currency,
                                child: Text(currency),
                              );
                            }).toList(),
                            isExpanded: true,
                          ),
                        );
                      },
                    ),
                  ),
                ],
              ),
              SizedBox(height: 16.0),
              Text(
                'Amount:',
                style: TextStyle(
                  fontSize: 18.0,
                  fontWeight: FontWeight.bold,
                ),
              ),
              SizedBox(height: 8.0),
              TextField(
                keyboardType: TextInputType.number,
                decoration: InputDecoration(
                  labelText: 'Amount',
                  border: OutlineInputBorder(),
                ),
                onChanged: (value) {
                  setState(() {
                    inputAmount = double.tryParse(value) ?? 0;
                  });
                },
              ),
              SizedBox(height: 16.0),
              ElevatedButton(
                onPressed: convertCurrency,
                child: Text('Convert Currency'),
              ),
              SizedBox(height: 8.0),
              Row(
                children: [
                  Expanded(
                    child: Text(
                      'Conversion Result:',
                      style: TextStyle(
                        fontSize: 16.0,
                        fontWeight: FontWeight.bold,
                      ),
                    ),
                  ),
                  SizedBox(width: 8.0),
                  Text(
                    '$outputAmount $selectedCurrencyTo',
                    style: TextStyle(
                      fontSize: 16.0,
                      fontWeight: FontWeight.bold,
                    ),
                  ),
                ],
              ),
              SizedBox(height: 32.0),
              Text(
                'Convert Time',
                style: TextStyle(
                  fontSize: 18.0,
                  fontWeight: FontWeight.bold,
                ),
              ),
              SizedBox(height: 8.0),
              Container(
                padding: EdgeInsets.symmetric(horizontal: 12.0),
                decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(8.0),
                  border: Border.all(color: Colors.grey),
                ),
                child: DropdownButton<String>(
                  value: selectedTimeZone,
                  onChanged: (String? newValue) {
                    setState(() {
                      selectedTimeZone = newValue ?? '';
                    });
                  },
                  items: timeZones.map((String timeZone) {
                    return DropdownMenuItem<String>(
                      value: timeZone,
                      child: Text(timeZone),
                    );
                  }).toList(),
                ),
              ),
              SizedBox(height: 16.0),
              Text(
                'Selected Time: ${selectedTime.toString()}',
                style: TextStyle(
                  fontSize: 16.0,
                  fontWeight: FontWeight.bold,
                ),
              ),
              SizedBox(height: 16.0),
              Text(
                'Converted Time:',
                style: TextStyle(
                  fontSize: 16.0,
                  fontWeight: FontWeight.bold,
                ),
              ),
              SizedBox(height: 8.0),
              Text(
                convertTime(selectedTimeZone),
                style: TextStyle(
                  fontSize: 16.0,
                  fontWeight: FontWeight.bold,
                ),
              ),
            ],
          ),
        ),
      ),
    );
  }
}
